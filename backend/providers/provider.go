...
type Provider interface {
	...
	Search(manga_name string) ([]structs.MangaProps, error)

	// Function to get all Chapters from certain Provider
	GetAllChapters(url string) ([]structs.Chapter, error)
...
func (p *ProviderFactory) FullSearch(s string) map[string][]structs.MangaProps {

	response := make(map[string][]structs.MangaProps)
